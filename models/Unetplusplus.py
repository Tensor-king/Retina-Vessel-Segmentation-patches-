...
# 当我们采用from archs import * 时，没有在__all__里面的属性和方法导出不了




# in_channels -> out_channels 不改变图片大小
    ...
    def forward(self, x):
        ...
        if len(x)>2:
            if x[1].shape != x[2].shape:
                print("尺寸出错")
        # [N, C, H, W]
        diff_y = x[1].size()[2] - x1.size()[2]
        diff_x = x[1].size()[3] - x1.size()[3]
...
class Unetplusplus(nn.Module):
    ...
    def forward(self, input):
        ...
        x1_2 = self.conv1_2(self.up([x2_1,x1_1,x1_0]))
